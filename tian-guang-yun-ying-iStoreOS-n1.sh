...
#天光云影app云同步备份文件远程助手
# 定义变量 - N1盒子iStoreOS专用（ARM架构）
USERNAME="admin"               # 账号
PASSWORD="admin"         # 密码
CONFIG_DIR="/mnt/mydisk/docker/tgyy-config"  # 你的docker目录
PORT_MAPPING="5000:5000"     # 端口映射
PROJECT_DIR="/mnt/mydisk/docker/tgyy-web"    # 你的docker目录

# 检查Docker是否安装，未安装则尝试安装
if ! command -v docker &> /dev/null; then
    echo "未检测到Docker，尝试安装..."
    opkg update || { echo "更新软件源失败"; exit 1; }
    opkg install docker luci-app-docker || { echo "安装Docker失败，请手动在iStoreOS界面安装Docker"; exit 1; }
    /etc/init.d/docker start
    usermod -aG docker root
fi

# 创建项目目录（如果不存在）
mkdir -p "$PROJECT_DIR" || { echo "无法创建项目目录 $PROJECT_DIR"; exit 1; }
...
<head>
    ...
    <style>
        ...
        .link-section { margin: 20px 0; padding: 15px; background-color: #f5f5f4; border-radius: 6px; }
...
# 创建Dockerfile（N1盒子ARM架构专用）
cat > Dockerfile << 'EOF'
# 使用ARM32v7架构Python镜像（适配N1盒子）
...
CMD ["nice", "-n", "10", "python", "app.py"]
EOF

# 创建完整配置文件（更新为包含所有扩展配置项的版本）
...
# 配置Docker镜像源和DNS
echo '{"registry-mirrors": ["https://mirror.baidubce.com", "https://ccr.ccs.tencentyun.com"],"dns": ["114.114.114.114", "223.5.5.5"]}' > /etc/docker/daemon.json
...
# 多源拉取ARM架构基础镜像
echo "尝试拉取ARM架构基础镜像..."
if ! docker pull arm32v7/python:3.9-slim; then
    echo "尝试腾讯云镜像..."
    if ! docker pull ccr.ccs.tencentyun.com/arm32v7/python:3.9-slim; then
        docker tag ccr.ccs.tencentyun.com/arm32v7/python:3.9-slim arm32v7/python:3.9-slim
    else
        echo "尝试百度云镜像..."
        if ! docker pull mirror.baidubce.com/arm32v7/python:3.9-slim; then
            docker tag mirror.baidubce.com/arm32v7/python:3.9-slim arm32v7/python:3.9-slim
        else
            echo "手动下载镜像方案..."
            opkg install wget
            wget https://github.com/balena-io-library/base-images/releases/download/v2023.09.1/arm32v7-python-3.9-slim.tar.gz -O /tmp/python.tar.gz
            docker load -i /tmp/python.tar.gz
            docker tag arm32v7/python:3.9-slim arm32v7/python:3.9-slim
...
# 构建并启动
docker build --platform linux/arm/v7 --network=host -t tgyy-web-config . || { echo "构建失败"; exit 1; }
docker run -d \
  --name tgyy-web \
...
# 部署完成信息
echo "部署完成！"
echo "访问地址: http://$(uci get network.lan.ipaddr):${PORT_MAPPING%:*}/"
echo "用户名: $USERNAME"
echo "密码: $PASSWORD"
echo "配置文件路径: $CONFIG_DIR/all_configs.json"
